...
class EmailsTemplate extends CI_Controller {
	...
	public function __construct() {
		...
		$this->userId = md5($aSess['session_id'].date('Ymd')); //dodanie do indentyfikatora sesji daty - problem z powtarzajacymi sie id
	...
	public function pdf($type='I') {
		...
		$pdf->Image('uploads/logo_pdf.jpg');
		$pdf->Ln(2);
		$pdf->Image('uploads/u'.$this->userId.'_saved_pdf.jpg', '43');
	...
	public function save_item() {
		...
		foreach ($a as $element) {
			...
			$row = $query->row_array();
			$t[$element] = isset($row['title']) ? $row['title'] : '';
		}
	...
	public function ajax_save_image()
	{
		...
		$this->UI->imagesave($bg, 'uploads/u'.$this->userId.'_saved_pdf', 'JPG', $aInfo);
	...
	public function send() {
		...
		$my_message = '
			<b>Witam serdecznie</b><br><br>
			Zapytanie ze strony WWW  - Konfigurator<br><br>
			<b style="color:red">Dane osoby zadającej pytanie:</b><br>
			'.$my_name.' '.$my_mail.'<br><br>
			<b style="color:red">Wiadomość:</b><br>
			'.strip_tags($this->input->post('my_text')).'<br><br>
			<b>Pozdrawiam!</b>';
