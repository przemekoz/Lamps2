...
class EmailsTemplate extends CI_Controller {
	...
	public function save_item() {
		...
		list($column, $crown, $fitting) = $this->get_data_elelemnts(array($idColumn, $idCrown, $idFitting));
		...
		if ($idCrown) {
			/* dane korony */
			$q = $this->db->select('width, height, mode')->get_where('crown', array('id'=>$idCrown));
			$crown = $q->result_array();
			$crown = $crown[0];
			/* zwiekszamy wysokosc o wysokosc kolumny */
			$height += $crown['height'];
		...
		//wczytaj i dodaj do nowego obrazka oprawe - w koejnym kroku będzie odpowiednio umieszczona na obrazku
